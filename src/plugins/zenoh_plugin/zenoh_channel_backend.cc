...
void ZenohChannelBackend::Initialize(YAML::Node options_node) {
  AIMRT_CHECK_ERROR_THROW(std::atomic_exchange(&state_, State::kInit) == State::kPreInit, "Zenoh channel backend can only be initialized once.");
...
void ZenohChannelBackend::Start() { AIMRT_CHECK_ERROR_THROW(std::atomic_exchange(&state_, State::kStart) == State::kInit, "Method can only be called when state is 'Init'."); }

void ZenohChannelBackend::Shutdown() {
  if (std::atomic_exchange(&state_, State::kShutdown) == State::kShutdown) return;
}

bool ZenohChannelBackend::RegisterPublishType(const runtime::core::channel::PublishTypeWrapper& publish_type_wrapper) noexcept {
  try {
    ...
    std::string pattern = std::string("channel/") + util::UrlEncode(info.topic_name) + "/" + util::UrlEncode(info.msg_type) + limit_domain_;
...
bool ZenohChannelBackend::Subscribe(const runtime::core::channel::SubscribeWrapper& subscribe_wrapper) noexcept {
  try {
    AIMRT_CHECK_ERROR_THROW(state_.load() == State::kInit, "Method can only be called when state is 'Init'.");
    ...
    auto handle = [this, topic_name = info.topic_name, sub_tool_ptr](const z_loaned_sample_t* message) {
      try {
...
void ZenohChannelBackend::Publish(runtime::core::channel::MsgWrapper& msg_wrapper) noexcept {
  try {
    ...
    std::string zenoh_pub_topic = std::string("channel/") + util::UrlEncode(info.topic_name) + "/" + util::UrlEncode(info.msg_type) + limit_domain_;
