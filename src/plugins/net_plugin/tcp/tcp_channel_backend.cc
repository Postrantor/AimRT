...
void TcpChannelBackend::Initialize(YAML::Node options_node) {
  AIMRT_CHECK_ERROR_THROW(std::atomic_exchange(&state_, State::kInit) == State::kPreInit, "Tcp channel backend can only be initialized once.");
...
void TcpChannelBackend::Start() { AIMRT_CHECK_ERROR_THROW(std::atomic_exchange(&state_, State::kStart) == State::kInit, "Method can only be called when state is 'Init'."); }
...
bool TcpChannelBackend::RegisterPublishType(const runtime::core::channel::PublishTypeWrapper& publish_type_wrapper) noexcept {
  try {
    ...
    for (const auto& publish_add : server_url_list) {
      ...
      boost::asio::ip::tcp::endpoint ep{boost::asio::ip::make_address(v[0].c_str()), static_cast<uint16_t>(atoi(v[1].c_str()))};
...
bool TcpChannelBackend::Subscribe(const runtime::core::channel::SubscribeWrapper& subscribe_wrapper) noexcept {
  try {
    AIMRT_CHECK_ERROR_THROW(state_.load() == State::kInit, "Method can only be called when state is 'Init'.");
      ...
      boost::asio::co_spawn(
          *io_ptr_,
          [this, server_ep, msg_buf_ptr]() -> boost::asio::awaitable<void> {
            aimrt::common::net::AsioTcpClient::Options client_options{.svr_ep = server_ep};
