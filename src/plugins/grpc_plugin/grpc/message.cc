...
std::optional<GrpcMessagePrefix> ParseGrpcMessagePrefix(std::string_view message) {
  ...
  auto message_length = (static_cast<uint32_t>(static_cast<unsigned char>(message[1])) << 24) | (static_cast<uint32_t>(static_cast<unsigned char>(message[2])) << 16) |
                        (static_cast<uint32_t>(static_cast<unsigned char>(message[3])) << 8) | static_cast<uint32_t>(static_cast<unsigned char>(message[4]));
