...
void BindCpuForCurrentThread(const std::vector<uint32_t>& cpu_set) {
...
#if defined(_WIN32)
  throw aimrt::common::util::AimRTException("BindCpuForCurrentThread currently not supported on the Windows platform.");
#else
  static const uint32_t kMaxCpuIdx = std::thread::hardware_concurrency();
  for (auto cpu_idx : cpu_set) {
    if (cpu_idx >= kMaxCpuIdx) {
      throw aimrt::common::util::AimRTException(::aimrt_fmt::format("Invalid cpu index '{}', max cpu idx is '{}'.", cpu_idx, kMaxCpuIdx));
    }
  }
  ...
  if (ret) {
    throw aimrt::common::util::AimRTException(::aimrt_fmt::format("Call 'pthread_setaffinity_np' get error, ret code '{}'", ret));
  }
#endif
}
...
void SetCpuSchedForCurrentThread(std::string_view sched) {
...
#if defined(_WIN32)
  throw aimrt::common::util::AimRTException("SetCpuSchedForCurrentThread currently not supported on the Windows platform.");
#else
  if (sched == "SCHED_OTHER") {
    ...
    if (ret) {
      throw aimrt::common::util::AimRTException(::aimrt_fmt::format("Call 'pthread_setschedparam' get error, ret code '{}'", ret));
    }
    ...
    if (pos == std::string_view::npos || pos == sched.size() - 1) {
      throw aimrt::common::util::AimRTException(::aimrt_fmt::format("Invalid sched parm '{}'", sched));
    }
    ...
    if (param.sched_priority < priority_min || param.sched_priority > priority_max) {
      throw aimrt::common::util::AimRTException(::aimrt_fmt::format("Invalid sched priority '{}', required range {}~{}", param.sched_priority, priority_min, priority_max));
    }
    ...
    if (ret) {
      throw aimrt::common::util::AimRTException(::aimrt_fmt::format("Call 'pthread_getschedparam' get error, ret code '{}'", ret));
    }
    if (check_policy != policy || check_param.sched_priority != param.sched_priority) {
      throw aimrt::common::util::AimRTException(
          ::aimrt_fmt::format("Set sched failed, want '{}:{}', actual '{}:{}'", policy, param.sched_priority, check_policy, check_param.sched_priority));
    }
  }
