...
public:
  explicit ParameterHandleProxy(ParameterHandle& parameter_handle) : base_(GenBase(&parameter_handle)) {}
  ...
  static aimrt_parameter_handle_base_t GenBase(void* impl) {
    return aimrt_parameter_handle_base_t{
        .get_parameter = [](void* impl, aimrt_string_view_t key) -> aimrt_parameter_val_view_holder_t {
          ...
          if (!ptr) [[unlikely]] {
            return aimrt_parameter_val_view_holder_t{.parameter_val = aimrt_string_view_t{nullptr, 0}, .release_callback = nullptr};
          }

          auto* f = new aimrt::parameter::ParameterValReleaseCallback();
          (*f) = [ptr, f]() { delete f; };

          return aimrt_parameter_val_view_holder_t{.parameter_val = aimrt::util::ToAimRTStringView(*ptr), .release_callback = f->NativeHandle()};
        },
        .set_parameter =
