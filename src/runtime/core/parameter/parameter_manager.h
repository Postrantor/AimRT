...
public:
  ParameterManager() : logger_ptr_(std::make_shared<aimrt::common::util::LoggerWrapper>()) {}
  ...
  const ParameterHandleProxy& GetParameterHandleProxy(std::string_view module_name = "core") {
    return GetParameterHandleProxy(util::ModuleDetailInfo{.name = std::string(module_name), .pkg_path = "core"});
  }
...
private:
  ...
  public:
    ParameterHandleProxyWrap() : parameter_handle_proxy(parameter_handle) {}

    ParameterHandle parameter_handle;
    ParameterHandleProxy parameter_handle_proxy;
  };

  std::unordered_map<std::string, std::unique_ptr<ParameterHandleProxyWrap>, aimrt::common::util::StringHash, std::equal_to<>> parameter_handle_proxy_wrap_map_;
};
