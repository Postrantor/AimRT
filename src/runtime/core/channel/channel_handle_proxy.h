...
private:
  bool RegisterPublishType(const aimrt_type_support_base_t* msg_type_support) {
    return channel_backend_manager_.RegisterPublishType(
        RegisterPublishTypeProxyInfoWrapper{.pkg_path = pkg_path_, .module_name = module_name_, .topic_name = topic_name_, .msg_type_support = msg_type_support});
  }

  void Publish(aimrt_string_view_t msg_type, const aimrt_channel_context_base_t* ctx_ptr, const void* msg_ptr) {
    channel_backend_manager_.Publish(
        PublishProxyInfoWrapper{.pkg_path = pkg_path_, .module_name = module_name_, .topic_name = topic_name_, .msg_type = msg_type, .ctx_ptr = ctx_ptr, .msg_ptr = msg_ptr});
  }

  void MergeSubscribeContextToPublishContext(const aimrt_channel_context_base_t* subscribe_ctx_ptr, const aimrt_channel_context_base_t* publish_ctx_ptr) {
  ...
  static aimrt_channel_publisher_base_t GenBase(void* impl) {
    return aimrt_channel_publisher_base_t{
        ...
        .publish =
            [](void* impl, aimrt_string_view_t msg_type, const aimrt_channel_context_base_t* ctx_ptr, const void* msg_ptr) {
              static_cast<PublisherProxy*>(impl)->Publish(msg_type, ctx_ptr, msg_ptr);  //
            },
        .get_topic = [](void* impl) -> aimrt_string_view_t { return aimrt::util::ToAimRTStringView(static_cast<PublisherProxy*>(impl)->topic_name_); },
...
public:
  SubscriberProxy(std::string_view pkg_path, std::string_view module_name, std::string_view topic_name, ChannelBackendManager& channel_backend_manager)
      : pkg_path_(pkg_path),
        module_name_(module_name),
        topic_name_(topic_name),
        channel_backend_manager_(channel_backend_manager),
...
private:
  bool Subscribe(const aimrt_type_support_base_t* msg_type_support, aimrt_function_base_t* callback) {
    return channel_backend_manager_.Subscribe(
        SubscribeProxyInfoWrapper{.pkg_path = pkg_path_, .module_name = module_name_, .topic_name = topic_name_, .msg_type_support = msg_type_support, .callback = callback});
  }
  ...
  static aimrt_channel_subscriber_base_t GenBase(void* impl) {
    return aimrt_channel_subscriber_base_t{
        ...
        .get_topic = [](void* impl) -> aimrt_string_view_t { return aimrt::util::ToAimRTStringView(static_cast<SubscriberProxy*>(impl)->topic_name_); },
        .impl = impl};
  }
...
public:
  using PublisherProxyMap = std::unordered_map<std::string, std::unique_ptr<PublisherProxy>, aimrt::common::util::StringHash, std::equal_to<>>;

  using SubscriberProxyMap = std::unordered_map<std::string, std::unique_ptr<SubscriberProxy>, aimrt::common::util::StringHash, std::equal_to<>>;
...
private:
  const aimrt_channel_publisher_base_t* GetPublisher(aimrt_string_view_t input_topic) {
    ...
    auto emplace_ret =
        publisher_proxy_map_.emplace(topic, std::make_unique<PublisherProxy>(pkg_path_, module_name_, topic, logger_, channel_backend_manager_, passed_context_meta_keys_));
  ...
  const aimrt_channel_subscriber_base_t* GetSubscriber(aimrt_string_view_t input_topic) {
    ...
    auto emplace_ret = subscriber_proxy_map_.emplace(topic, std::make_unique<SubscriberProxy>(pkg_path_, module_name_, topic, channel_backend_manager_));
  ...
  static aimrt_channel_handle_base_t GenBase(void* impl) {
    return aimrt_channel_handle_base_t{
        .get_publisher = [](void* impl, aimrt_string_view_t topic) -> const aimrt_channel_publisher_base_t* { return static_cast<ChannelHandleProxy*>(impl)->GetPublisher(topic); },
