...
class ExecutorProxy {
public:
  explicit ExecutorProxy(ExecutorBase* executor_ptr) : base_(GenBase(executor_ptr)) {}
...
private:
  static aimrt_executor_base_t GenBase(void* impl) {
    return aimrt_executor_base_t{
        .type = [](void* impl) -> aimrt_string_view_t { return aimrt::util::ToAimRTStringView(static_cast<ExecutorBase*>(impl)->Type()); },
        .name = [](void* impl) -> aimrt_string_view_t { return aimrt::util::ToAimRTStringView(static_cast<ExecutorBase*>(impl)->Name()); },
        .is_thread_safe = [](void* impl) -> bool { return static_cast<ExecutorBase*>(impl)->ThreadSafe(); },
        .is_in_current_executor = [](void* impl) -> bool { return static_cast<ExecutorBase*>(impl)->IsInCurrentExecutor(); },
        .is_support_timer_schedule = [](void* impl) -> bool { return static_cast<ExecutorBase*>(impl)->SupportTimerSchedule(); },
        .execute =
            [](void* impl, aimrt_function_base_t* task) {
              static_cast<ExecutorBase*>(impl)->Execute(aimrt::executor::Task(task));  //
            },
        .now = [](void* impl) -> uint64_t { return aimrt::common::util::GetTimestampNs(static_cast<ExecutorBase*>(impl)->Now()); },
        .execute_at_ns =
            [](void* impl, uint64_t tp, aimrt_function_base_t* task) {
              static_cast<ExecutorBase*>(impl)->ExecuteAt(aimrt::common::util::GetTimePointFromTimestampNs(tp), aimrt::executor::Task(task));  //
            },
        .impl = impl};
  }
...
public:
  using ExecutorProxyMap = std::unordered_map<std::string, std::unique_ptr<ExecutorProxy>, aimrt::common::util::StringHash, std::equal_to<>>;

public:
  explicit ExecutorManagerProxy(const ExecutorProxyMap& executor_proxy_map) : executor_proxy_map_(executor_proxy_map), base_(GenBase(this)) {}
