...
void AsioThreadExecutor::Initialize(std::string_view name, YAML::Node options_node) {
  AIMRT_CHECK_ERROR_THROW(std::atomic_exchange(&state_, State::kInit) == State::kPreInit, "AsioThreadExecutor can only be initialized once.");
  ...
  AIMRT_CHECK_ERROR_THROW(options_.thread_num > 0, "Invalide asio thread executor options, thread num is zero.");

  io_ptr_ = std::make_unique<asio::io_context>(options_.thread_num);
  work_guard_ptr_ = std::make_unique<asio::executor_work_guard<asio::io_context::executor_type>>(io_ptr_->get_executor());
  ...
  for (uint32_t ii = 0; ii < options_.thread_num; ++ii) {
    threads_.emplace_back([this, ii] {
      ...
      try {
        ...
        AIMRT_WARN("Set thread policy for asio thread executor '{}' get exception, {}", Name(), e.what());
      }
      ...
      try {
        ...
        AIMRT_FATAL("Asio thread executor '{}' run loop get exception, {}", Name(), e.what());
      }
...
void AsioThreadExecutor::Start() { AIMRT_CHECK_ERROR_THROW(std::atomic_exchange(&state_, State::kStart) == State::kInit, "Method can only be called when state is 'Init'."); }
...
void AsioThreadExecutor::Execute(aimrt::executor::Task&& task) noexcept {
  if (state_.load() != State::kInit && state_.load() != State::kStart) [[unlikely]] {
    fprintf(stderr, "Asio thread executor '%s' can only execute task when state is 'Init' or 'Start'.\n", name_.c_str());
    return;
  }
  ...
  if (cur_queue_task_num > queue_threshold_) [[unlikely]] {
    fprintf(stderr, "The number of tasks in the asio thread executor '%s' has reached the threshold '%u', the task will not be delivered.\n", name_.c_str(), queue_threshold_);
    --queue_task_num_;
    return;
  }

  if (cur_queue_task_num > queue_warn_threshold_) [[unlikely]] {
    fprintf(stderr, "The number of tasks in the asio thread executor '%s' is about to reach the threshold: '%u / %u'.\n", name_.c_str(), cur_queue_task_num, queue_threshold_);
  }
...
void AsioThreadExecutor::ExecuteAt(std::chrono::system_clock::time_point tp, aimrt::executor::Task&& task) noexcept {
  if (state_.load() != State::kInit && state_.load() != State::kStart) [[unlikely]] {
    fprintf(stderr, "Asio thread executor '%s' can only execute task when state is 'Init' or 'Start'.\n", name_.c_str());
    return;
  }
  ...
  try {
    ...
    timer_ptr->async_wait([this, timer_ptr, task{std::move(task)}](asio::error_code ec) {
      if (ec) [[unlikely]] {
        AIMRT_ERROR("Asio thread executor '{}' timer get err, code '{}', msg: {}", Name(), ec.value(), ec.message());
        return;
      }
