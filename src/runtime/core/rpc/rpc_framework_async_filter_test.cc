...
TEST(RPC_FRAMEWORK_ASYNC_FILTER_TEST, FrameworkAsyncRpcFilterCollector_base) {
  ...
  auto rpc_handle = [](const std::shared_ptr<InvokeWrapper> &invoke_wrapper_ptr) {
    *static_cast<std::string *>(invoke_wrapper_ptr->rsp_ptr) = *static_cast<const std::string *>(invoke_wrapper_ptr->req_ptr);
  ...
  auto invoke_wrapper_ptr = std::make_shared<InvokeWrapper>(InvokeWrapper{.info = info, .req_ptr = &req, .rsp_ptr = &rsp});

  aimrt::rpc::Context ctx;
  invoke_wrapper_ptr->ctx_ref = ctx;

  std::promise<aimrt::rpc::Status> status_promise;
  invoke_wrapper_ptr->callback = [&status_promise](aimrt::rpc::Status status) mutable { status_promise.set_value(status); };
...
TEST(RPC_FRAMEWORK_ASYNC_FILTER_TEST, FrameworkAsyncRpcFilterCollector_multiple_filters) {
  ...
  // 先注册的在内层
  FrameworkAsyncRpcFilter filter_1 = [](const std::shared_ptr<InvokeWrapper> &invoke_wrapper_ptr, FrameworkAsyncRpcHandle &&h) {
    std::thread([invoke_wrapper_ptr, h{std::move(h)}]() {
      ...
      invoke_wrapper_ptr->callback = [ctx_ref, callback{std::move(invoke_wrapper_ptr->callback)}](aimrt::rpc::Status status) mutable {
  ...
  // 后注册的在外层
  FrameworkAsyncRpcFilter filter_2 = [](const std::shared_ptr<InvokeWrapper> &invoke_wrapper_ptr, FrameworkAsyncRpcHandle &&h) {
    std::thread([invoke_wrapper_ptr, h{std::move(h)}]() {
      auto ctx_ref = invoke_wrapper_ptr->ctx_ref;
  ...
  std::promise<aimrt::rpc::Status> status_promise;
  invoke_wrapper_ptr->callback = [&ctx, &status_promise](aimrt::rpc::Status status) mutable {
