...
void LocalRpcBackend::Initialize(YAML::Node options_node) {
  AIMRT_CHECK_ERROR_THROW(std::atomic_exchange(&state_, State::kInit) == State::kPreInit, "Local rpc backend can only be initialized once.");

  if (options_node && !options_node.IsNull()) options_ = options_node.as<Options>();

  client_tool_ptr_ = std::make_unique<util::RpcClientTool<std::shared_ptr<InvokeWrapper>>>();
  ...
  if (!options_.timeout_executor.empty()) {
    ...
    client_tool_ptr_->RegisterTimeoutHandle(
        [](std::shared_ptr<InvokeWrapper>&& client_invoke_wrapper_ptr) { client_invoke_wrapper_ptr->callback(aimrt::rpc::Status(AIMRT_RPC_STATUS_TIMEOUT)); });

    AIMRT_TRACE("Local rpc backend enable the timeout function, use '{}' as timeout executor.", options_.timeout_executor);
  } else {
...
void LocalRpcBackend::Start() { AIMRT_CHECK_ERROR_THROW(std::atomic_exchange(&state_, State::kStart) == State::kInit, "Method can only be called when state is 'Init'."); }
...
bool LocalRpcBackend::RegisterServiceFunc(const ServiceFuncWrapper& service_func_wrapper) noexcept {
  try {
...
bool LocalRpcBackend::RegisterClientFunc(const ClientFuncWrapper& client_func_wrapper) noexcept {
  try {
    if (state_.load() != State::kInit) {
...
void LocalRpcBackend::Invoke(const std::shared_ptr<InvokeWrapper>& client_invoke_wrapper_ptr) noexcept {
  try {
    ...
    if (service_pkg_path.empty()) {
      if (service_module_name.empty()) {
        ...
        auto service_func_register_index_find_pkg_itr = service_func_register_index_find_func_itr->second.begin();
        ...
        if (service_pkg_path.empty()) {
          AIMRT_WARN("Can not find service func '{}' in module '{}'. Addr: {}", client_info.func_name, service_module_name, to_addr);
      ...
      auto service_func_register_index_find_pkg_itr = service_func_register_index_find_func_itr->second.find(service_pkg_path);

      if (service_func_register_index_find_pkg_itr == service_func_register_index_find_func_itr->second.end()) {
        AIMRT_WARN("Can not find service func '{}' in pkg '{}'. Addr: {}", client_info.func_name, service_pkg_path, to_addr);
      ...
      if (service_module_name.empty()) {
        ...
        auto service_func_register_index_find_module_itr = service_func_register_index_find_pkg_itr->second.find(service_module_name);

        if (service_func_register_index_find_module_itr == service_func_register_index_find_pkg_itr->second.end()) {
          AIMRT_WARN("Can not find service func '{}' in pkg '{}' module '{}'. Addr: {}", client_info.func_name, service_pkg_path, service_module_name, to_addr);
    ...
    AIMRT_TRACE("Invoke rpc func '{}' in pkg '{}' module '{}'.", client_info.func_name, service_pkg_path, service_module_name);

    // 找注册的service方法
    const auto* service_func_wrapper_ptr = rpc_registry_ptr_->GetServiceFuncWrapperPtr(client_info.func_name, service_pkg_path, service_module_name);

    if (service_func_wrapper_ptr == nullptr) [[unlikely]] {
      AIMRT_WARN("Can not find service func '{}' in pkg '{}' module '{}'", client_info.func_name, service_pkg_path, service_module_name);
    ...
    auto service_invoke_wrapper_ptr = std::make_shared<InvokeWrapper>(InvokeWrapper{.info = service_func_wrapper_ptr->info});
    ...
    for (const auto& item : meta_keys) ctx_ptr->SetMetaValue(item, client_invoke_wrapper_ptr->ctx_ref.GetMetaValue(item));
    ...
    if (service_pkg_path == client_info.pkg_path) {
      ...
      service_invoke_wrapper_ptr->callback = [ctx_ptr, service_invoke_wrapper_ptr, client_invoke_wrapper_ptr](aimrt::rpc::Status status) {
    ...
    if (!buffer_array_view_ptr) [[unlikely]] {
      // 序列化失败
      AIMRT_ERROR(
          "Req serialization failed in local rpc backend, serialization_type {}, pkg_path: {}, module_name: {}, func_name: {}", serialization_type, client_info.pkg_path,
          client_info.module_name, client_info.func_name);
    ...
    bool deserialize_ret = service_info.req_type_support_ref.Deserialize(serialization_type, *(buffer_array_view_ptr->NativeHandle()), service_req_ptr.get());

    if (!deserialize_ret) [[unlikely]] {
      // 反序列化失败
      AIMRT_FATAL(
          "Rsp deserialization failed in local rpc backend, serialization_type {}, pkg_path: {}, module_name: {}, func_name: {}", serialization_type, service_info.pkg_path,
          service_info.module_name, service_info.func_name);
    ...
    // 设置回调
    service_invoke_wrapper_ptr->callback = [this, service_invoke_wrapper_ptr, ctx_ptr, cur_req_id, service_req_ptr, service_rsp_ptr,
      ...
      if (!buffer_array_view_ptr) [[unlikely]] {
        // 序列化失败
        AIMRT_ERROR(
            "Rsp serialization failed in local rpc backend, serialization_type {}, pkg_path: {}, module_name: {}, func_name: {}", serialization_type, service_info.pkg_path,
            service_info.module_name, service_info.func_name);
      ...
      // client rsp 反序列化
      bool deserialize_ret = client_info.rsp_type_support_ref.Deserialize(serialization_type, *(buffer_array_view_ptr->NativeHandle()), client_invoke_wrapper_ptr->rsp_ptr);

      if (!deserialize_ret) {
        // 反序列化失败
        AIMRT_ERROR(
            "Req deserialization failed in local rpc backend, serialization_type {}, pkg_path: {}, module_name: {}, func_name: {}", serialization_type, client_info.pkg_path,
            client_info.module_name, client_info.func_name);
...
void LocalRpcBackend::RegisterGetExecutorFunc(const std::function<aimrt::executor::ExecutorRef(std::string_view)>& get_executor_func) {
