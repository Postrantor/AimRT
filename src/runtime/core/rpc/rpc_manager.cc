  ...
  static bool decode(const Node& node, Options& rhs) {
    ...
    if (node["clients_options"] && node["clients_options"].IsSequence()) {
      for (const auto& client_options_node : node["clients_options"]) {
        ...
        if (client_options_node["enable_filters"]) client_options.enable_filters = client_options_node["enable_filters"].as<std::vector<std::string>>();
    ...
    if (node["servers_options"] && node["servers_options"].IsSequence()) {
      for (const auto& server_options_node : node["servers_options"]) {
        ...
        if (server_options_node["enable_filters"]) server_options.enable_filters = server_options_node["enable_filters"].as<std::vector<std::string>>();
...
void RpcManager::Initialize(YAML::Node options_node) {
  ...
  AIMRT_CHECK_ERROR_THROW(std::atomic_exchange(&state_, State::kInit) == State::kPreInit, "Rpc manager can only be initialized once.");
  ...
  for (auto& backend_options : options_.backends_options) {
    auto finditr = std::find_if(rpc_backend_vec_.begin(), rpc_backend_vec_.end(), [&backend_options](const auto& ptr) { return ptr->Name() == backend_options.type; });

    AIMRT_CHECK_ERROR_THROW(finditr != rpc_backend_vec_.end(), "Invalid rpc backend type '{}'", backend_options.type);
  ...
  for (const auto& item : options_.clients_options) {
    for (const auto& backend_name : item.enable_backends) {
      AIMRT_CHECK_ERROR_THROW(
          std::find(rpc_backend_name_vec.begin(), rpc_backend_name_vec.end(), backend_name) != rpc_backend_name_vec.end(), "Invalid rpc backend type '{}' for func '{}'",
...
std::list<std::pair<std::string, std::string>> RpcManager::GenInitializationReport() const {
  ...
  std::vector<std::vector<std::string>> client_info_table = {{"func", "module", "backends", "filters"}};
  ...
  for (const auto& client_index_itr : client_index_map) {
    ...
    AIMRT_CHECK_ERROR_THROW(client_backend_itr != client_backend_info.end(), "Invalid rpc registry info.");
  ...
  std::vector<std::vector<std::string>> server_info_table = {{"func", "module", "backends", "filters"}};
  ...
  for (const auto& server_index_itr : server_index_map) {
    ...
    AIMRT_CHECK_ERROR_THROW(server_backend_itr != server_backend_info.end(), "Invalid rpc registry info.");
  ...
  for (const auto& item : rpc_backend_vec_) rpc_backend_name_vec.emplace_back(item->Name());
...
void RpcManager::RegisterRpcBackend(std::unique_ptr<RpcBackendBase>&& rpc_backend_ptr) {
...
void RpcManager::RegisterGetExecutorFunc(const std::function<executor::ExecutorRef(std::string_view)>& get_executor_func) {
  AIMRT_CHECK_ERROR_THROW(state_.load() == State::kPreInit, "Method can only be called when state is 'PreInit'.");
...
const RpcHandleProxy& RpcManager::GetRpcHandleProxy(const util::ModuleDetailInfo& module_info) {
  ...
  auto emplace_ret =
      rpc_handle_proxy_map_.emplace(module_info.name, std::make_unique<RpcHandleProxy>(module_info.pkg_path, module_info.name, rpc_backend_manager_, passed_context_meta_keys_));
...
void RpcManager::RegisterLocalRpcBackend() {
  ...
  static_cast<LocalRpcBackend*>(local_rpc_backend_ptr.get())->SetLogger(logger_ptr_);

  static_cast<LocalRpcBackend*>(local_rpc_backend_ptr.get())->RegisterGetExecutorFunc(get_executor_func_);

  RegisterRpcBackend(std::move(local_rpc_backend_ptr));
}

void RpcManager::RegisterDebugLogFilter() {
  RegisterClientFilter("debug_log", [this](const std::shared_ptr<InvokeWrapper>& ptr, FrameworkAsyncRpcHandle&& h) {
    auto buf_ptr = TrySerializeReqWithCache(*ptr, "json");

    if (buf_ptr) {
      auto req_str = buf_ptr->JoinToString();

      AIMRT_INFO("RPC client start new rpc call. func name: {}, context: {}, req: {}", ptr->info.func_name, ptr->ctx_ref.ToString(), req_str);
    } else {
      AIMRT_INFO("RPC client start new rpc call. func name: {}, context: {}", ptr->info.func_name, ptr->ctx_ref.ToString());
    }

    ptr->callback = [this, ptr, callback{std::move(ptr->callback)}](aimrt::rpc::Status status) {
      if (!status.OK()) {
        AIMRT_WARN("RPC client get rpc error ret. func name: {}, status: {}", ptr->info.func_name, status.ToString());
      } else {
        ...
        if (buf_ptr) {
          ...
          AIMRT_INFO("RPC client get rpc ret. func name: {}, status: {}, rsp: {}", ptr->info.func_name, status.ToString(), rsp_str);
        } else {
          AIMRT_INFO("RPC client get rpc ret. func name: {}, status: {}", ptr->info.func_name, status.ToString());
        }
      }
  ...
  RegisterServerFilter("debug_log", [this](const std::shared_ptr<InvokeWrapper>& ptr, FrameworkAsyncRpcHandle&& h) {
    auto buf_ptr = TrySerializeReqWithCache(*ptr, "json");

    if (buf_ptr) {
      auto req_str = buf_ptr->JoinToString();

      AIMRT_INFO("RPC server start new rpc call. func name: {}, context: {}, req: {}", ptr->info.func_name, ptr->ctx_ref.ToString(), req_str);
    } else {
      AIMRT_INFO("RPC server start new rpc call. func name: {}, context: {}", ptr->info.func_name, ptr->ctx_ref.ToString());
    }

    ptr->callback = [this, ptr, callback{std::move(ptr->callback)}](aimrt::rpc::Status status) {
      if (!status.OK()) {
        AIMRT_WARN("RPC server get rpc error ret. func name: {}, status: {}", ptr->info.func_name, status.ToString());
      } else {
        auto buf_ptr = TrySerializeRspWithCache(*ptr, "json");
        ...
        if (buf_ptr) {
          ...
          AIMRT_INFO("RPC server get rpc ret. func name: {}, status: {}, rsp: {}", ptr->info.func_name, status.ToString(), rsp_str);
        } else {
          AIMRT_INFO("RPC server get rpc ret. func name: {}, status: {}", ptr->info.func_name, status.ToString());
        }
      }
