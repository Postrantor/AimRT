...
public:
  Context(aimrt_channel_context_type_t type = aimrt_channel_context_type_t::AIMRT_CHANNEL_PUBLISHER_CONTEXT)
      : type_(type),
        base_(aimrt_channel_context_base_t{.ops = GenOpsBase(), .impl = this}) {}
  ~Context() = default;
  ...
  std::string_view GetSerializationType() const { return GetMetaValue(AIMRT_CHANNEL_CONTEXT_KEY_SERIALIZATION_TYPE); }
  void SetSerializationType(std::string_view val) { SetMetaValue(AIMRT_CHANNEL_CONTEXT_KEY_SERIALIZATION_TYPE, val); }
...
private:
  static const aimrt_channel_context_base_ops_t* GenOpsBase() {
    static constexpr aimrt_channel_context_base_ops_t kOps{
        ...
        .get_type = [](void* impl) -> aimrt_channel_context_type_t { return static_cast<Context*>(impl)->type_; },
...
public:
  ...
  explicit ContextRef(const aimrt_channel_context_base_t* base_ptr) : base_ptr_(base_ptr) {}
  ~ContextRef() = default;

  explicit operator bool() const { return (base_ptr_ != nullptr); }

  const aimrt_channel_context_base_t* NativeHandle() const { return base_ptr_; }
