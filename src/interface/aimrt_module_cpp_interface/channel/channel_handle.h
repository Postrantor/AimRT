...
public:
  ...
  explicit PublisherRef(const aimrt_channel_publisher_base_t* base_ptr) : base_ptr_(base_ptr) {}
  ~PublisherRef() = default;

  explicit operator bool() const { return (base_ptr_ != nullptr); }

  const aimrt_channel_publisher_base_t* NativeHandle() const { return base_ptr_; }
  ...
  void MergeSubscribeContextToPublishContext(const ContextRef subscribe_ctx_ref, ContextRef publish_ctx_ref) const {
    AIMRT_ASSERT(base_ptr_, "Reference is null.");
    base_ptr_->merge_subscribe_context_to_publish_context(base_ptr_->impl, subscribe_ctx_ref.NativeHandle(), publish_ctx_ref.NativeHandle());
  }
...
public:
  ...
  explicit SubscriberRef(const aimrt_channel_subscriber_base_t* base_ptr) : base_ptr_(base_ptr) {}
  ~SubscriberRef() = default;

  explicit operator bool() const { return (base_ptr_ != nullptr); }

  const aimrt_channel_subscriber_base_t* NativeHandle() const { return base_ptr_; }
   ...
   */
  bool Subscribe(const aimrt_type_support_base_t* msg_type_support, SubscriberCallback&& callback) {
    AIMRT_ASSERT(base_ptr_, "Reference is null.");
...
public:
  ...
  explicit ChannelHandleRef(const aimrt_channel_handle_base_t* base_ptr) : base_ptr_(base_ptr) {}
  ~ChannelHandleRef() = default;
  ...
  PublisherRef GetPublisher(std::string_view topic) {
    AIMRT_ASSERT(base_ptr_, "Reference is null.");
    return PublisherRef(base_ptr_->get_publisher(base_ptr_->impl, aimrt::util::ToAimRTStringView(topic)));
  }

  SubscriberRef GetSubscriber(std::string_view topic) {
    AIMRT_ASSERT(base_ptr_, "Reference is null.");
    return SubscriberRef(base_ptr_->get_subscriber(base_ptr_->impl, aimrt::util::ToAimRTStringView(topic)));
  }
...
public:
  explicit PublisherProxyBase(PublisherRef publisher, std::string_view msg_type_name) : publisher_(publisher), msg_type_name_(msg_type_name) {}
  ...
  std::string_view GetTopic() const { return publisher_.GetTopic(); }

protected:
  void PublishImpl(ContextRef ctx_ref, const void* msg_ptr) { publisher_.Publish(msg_type_name_, ctx_ref, msg_ptr); }
...
public:
  explicit SubscriberProxyBase(SubscriberRef subscriber, std::string_view msg_type_name) : subscriber_(subscriber), msg_type_name_(msg_type_name) {}
  ...
  std::string_view GetTopic() const { return subscriber_.GetTopic(); }
