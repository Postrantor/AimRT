...
public:
  LoggerRef() = default;
  explicit LoggerRef(const aimrt_logger_base_t* base_ptr) : base_ptr_(base_ptr) {}
  ~LoggerRef() = default;
  ...
  void Log(uint32_t lvl, uint32_t line, uint32_t column, const char* file_name, const char* function_name, const char* log_data, size_t log_data_size) const {
    AIMRT_ASSERT(base_ptr_, "Reference is null.");
    base_ptr_->log(base_ptr_->impl, static_cast<aimrt_log_level_t>(lvl), line, column, file_name, function_name, log_data, log_data_size);
  };
...
inline LoggerRef GetSimpleLoggerRef() {
  static constexpr aimrt_logger_base_t kSimpleLogger{
      .get_log_level = [](void* impl) -> aimrt_log_level_t { return static_cast<aimrt_log_level_t>(aimrt::common::util::SimpleLogger::GetLogLevel()); },
      .log =
          [](void* impl, aimrt_log_level_t lvl, uint32_t line, uint32_t column, const char* file_name, const char* function_name, const char* log_data, size_t log_data_size) {
            aimrt::common::util::SimpleLogger::Log(static_cast<uint32_t>(lvl), line, column, file_name, function_name, log_data, log_data_size);  //
          }};
