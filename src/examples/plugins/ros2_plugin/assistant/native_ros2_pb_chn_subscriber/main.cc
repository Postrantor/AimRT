...
public:
  RosTestWrapperChannelSubscriber() : Node("native_ros2_pb_chn_subscriber") {
    subscription_ = this->create_subscription<ros2_plugin_proto::msg::RosMsgWrapper>(
        "test_topic/pb_3Aaimrt_2Eprotocols_2Eexample_2EExampleEventMsg", 10, [this](ros2_plugin_proto::msg::RosMsgWrapper::UniquePtr wrapper_msg) {
          if (wrapper_msg->serialization_type == "pb") {
            ...
            RCLCPP_WARN(get_logger(), "unsupport serialization_type: %s", wrapper_msg->serialization_type.c_str());
          }
        });
  }
