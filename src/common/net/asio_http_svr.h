...
public:
  ...
  using HttpHandle = std::function<Awaitable<HttpHandleStatus>(const Request<ReqBodyType>&, Response<RspBodyType>&, std::chrono::nanoseconds)>;
  ...
  struct Options {
    ...
    static Options Verify(const Options& verify_options) {
      ...
      if (options.max_session_num > Tcp::acceptor::max_listen_connections) options.max_session_num = Tcp::acceptor::max_listen_connections;
  ...
  void RegisterHttpHandleFunc(std::string_view pattern, HttpHandle<RspBodyType>&& handle) {
    ...
    http_dispatcher_ptr_->RegisterHttpHandle(pattern, Session::GenHttpDispatcherHandle(std::move(handle)));
  }
  ...
  void Initialize(const Options& options) {
    ...
    AIMRT_CHECK_ERROR_THROW(CheckListenAddr(options_.ep), "{} is already in use.", aimrt::common::util::SSToString(options_.ep));
  }
        ...
        [this, self]() -> Awaitable<void> {
          ...
          while (state_.load() == State::kStart) {
            try {
              ...
              auto session_ptr = std::make_shared<Session>(io_ptr_, logger_ptr_, http_dispatcher_ptr_);
              session_ptr->Initialize(session_options_ptr_);

              co_await acceptor_.async_accept(session_ptr->Socket(), boost::asio::use_awaitable);
              AIMRT_TRACE("Http svr accept a new connection, remote addr is {}", aimrt::common::util::SSToString(session_ptr->Socket().remote_endpoint()));
              session_ptr->Start();

              session_ptr_list_.emplace_back(session_ptr);

            } catch (const std::exception& e) {
              AIMRT_TRACE("Http svr accept connection get exception and exit, exception info: {}", e.what());
            }
          }
        ...
        [this, self]() -> Awaitable<void> {
          while (state_.load() == State::kStart) {
            try {
              ...
              AIMRT_TRACE("Http svr timer get exception and exit, exception info: {}", e.what());
            }
          }
    ...
    boost::asio::dispatch(mgr_strand_, [this, self]() {
      ...
      while (stop_step) {
        try {
          ...
          AIMRT_WARN("Http svr stop get exception at step {}, exception info: {}", stop_step, e.what());
          ++stop_step;
        }
      }
...
private:
  ...
  struct SessionOptions {
    explicit SessionOptions(const Options& options) : doc_root(options.doc_root), max_no_data_duration(options.max_no_data_duration) {}
  ...
  public:
    using Dispatcher = HttpDispatcher<Awaitable<void>(const std::shared_ptr<Session>&, const Request<ReqBodyType>&)>;

    Session(const std::shared_ptr<IOCtx>& io_ptr, const std::shared_ptr<aimrt::common::util::LoggerWrapper>& logger_ptr, const std::shared_ptr<Dispatcher>& http_dispatcher_ptr)
        : io_ptr_(io_ptr),
          ...
          [this, self]() -> Awaitable<void> {
            try {
              ...
              while (state_.load() == SessionState::kStart && !close_connect_flag_) {
                ...
                size_t read_data_size = co_await http::async_read(stream_, buffer, req_parser, boost::asio::use_awaitable);
                AIMRT_TRACE("Http svr session async read {} bytes from {}", read_data_size, RemoteAddr());
                ...
                if (!bad_req_check_ret.empty()) {
                  ...
                  AIMRT_WARN("Http svr session get bad request, remote addr {}, err msg: {}, close_connect_flag: {}", RemoteAddr(), bad_req_check_ret.data(), close_connect_flag_);
                  size_t write_data_size = co_await http::async_write(stream_, rsp, boost::asio::use_awaitable);
                  AIMRT_TRACE("Http svr session async write {} bytes to {}", write_data_size, RemoteAddr());
                  continue;
                }
                ...
                if (!url_struct) {
                  ...
                  AIMRT_WARN("Http svr session can not parse url: {}, remote addr {}, close_connect_flag: {}", RemoteAddr(), req.target().data(), close_connect_flag_);
                  size_t write_data_size = co_await http::async_write(stream_, rsp, boost::asio::use_awaitable);
                  AIMRT_TRACE("Http svr session async write {} bytes to {}", write_data_size, RemoteAddr());
                  continue;
                }
                ...
                if (session_options_ptr_->doc_root.empty()) {
                  ...
                  AIMRT_WARN("Http svr session get 404, path: '{}', remote addr {}, close_connect_flag: {}", url_struct->path, RemoteAddr(), close_connect_flag_);
                  size_t write_data_size = co_await http::async_write(stream_, rsp, boost::asio::use_awaitable);
                  AIMRT_TRACE("Http svr session async write {} bytes to {}", write_data_size, RemoteAddr());
                  continue;
                }
                ...
                if (ec) {
                  ...
                  AIMRT_WARN("Http svr session get server error, err msg: {}, remote addr {}, close_connect_flag: {}", ec.message(), RemoteAddr(), close_connect_flag_);
                  size_t write_data_size = co_await http::async_write(stream_, rsp, boost::asio::use_awaitable);
                  AIMRT_TRACE("Http svr session async write {} bytes to {}", write_data_size, RemoteAddr());
                  continue;
                }
                ...
                if (req.method() == http::verb::head) {
                  ...
                  AIMRT_TRACE("Http svr session get head request, remote addr {}, close_connect_flag: {}", RemoteAddr(), close_connect_flag_);
                  size_t write_data_size = co_await http::async_write(stream_, rsp, boost::asio::use_awaitable);
                  AIMRT_TRACE("Http svr session async write {} bytes to {}", write_data_size, RemoteAddr());
                  continue;
                }
                ...
                // 处理file请求
                Response<http::file_body> rsp{std::piecewise_construct, std::make_tuple(std::move(body)), std::make_tuple(http::status::ok, req.version())};
                rsp.set(http::field::content_type, MimeType(path));
                rsp.content_length(size);
                rsp.keep_alive(req.keep_alive());
                ...
                AIMRT_TRACE("Http svr session get file request, remote addr {}, close_connect_flag: {}", RemoteAddr(), close_connect_flag_);
                size_t write_data_size = co_await http::async_write(stream_, rsp, boost::asio::use_awaitable);
                AIMRT_TRACE("Http svr session async write {} bytes to {}", write_data_size, RemoteAddr());
              }
            } catch (const std::exception& e) {
      ...
      boost::asio::co_spawn(
          ...
          [this, self]() -> Awaitable<void> {
            try {
              ...
              AIMRT_TRACE("Http svr session timer get exception and exit, remote addr {}, exception info: {}", RemoteAddr(), e.what());
            }
      ...
      boost::asio::dispatch(session_socket_strand_, [this, self]() {
        ...
        while (stop_step) {
          try {
            ...
            AIMRT_TRACE("Http svr session stop get exception at step {}, remote addr {}, exception info: {}", stop_step, RemoteAddr(), e.what());
            ++stop_step;
          }
        }
      });
      ...
      boost::asio::dispatch(session_mgr_strand_, [this, self]() {
        ...
        while (stop_step) {
          try {
            ...
            AIMRT_WARN("Http svr session mgr stop get exception at step {}, remote addr {}, exception info: {}", stop_step, RemoteAddr(), e.what());
            ++stop_step;
          }
        }
      });
    ...
    static Dispatcher::HttpHandle GenHttpDispatcherHandle(HttpHandle<RspBodyType>&& handle) {
      return [handle{std::move(handle)}](const std::shared_ptr<Session>& session_ptr, const Request<ReqBodyType>& req) -> Awaitable<void> {
        ...
        if (req_timeout_itr != req.end()) {
          ...
          if (aimrt::common::util::IsDigitStr(timeout_str)) handle_timeout = std::chrono::seconds(atoi(timeout_str.data()));
        }

        std::string err_info;
        try {
          handle_status = co_await boost::asio::co_spawn(
              *(session_ptr->io_ptr_), [&]() -> Awaitable<AsioHttpServer::HttpHandleStatus> { return handle(req, handle_rsp, handle_timeout); }, boost::asio::use_awaitable);

          if (handle_status != HttpHandleStatus::kOk) err_info = "handle failed, status: " + std::to_string(static_cast<uint8_t>(handle_status));
        } catch (const std::exception& e) {
          err_info = e.what();
        }
        ...
        if (!err_info.empty()) [[unlikely]] {
          ...
          AIMRT_HL_WARN(
              session_ptr->GetLogger(), "Http svr session custom handle request get exp, error info: {}, remote addr {}, close_connect_flag: {}", err_info,
              session_ptr->RemoteAddr(), session_ptr->close_connect_flag_);

          co_await boost::beast::http::async_write(session_ptr->stream_, rsp, boost::asio::use_awaitable);
          AIMRT_HL_TRACE(session_ptr->GetLogger(), "Http svr session async write to {} suc.", session_ptr->RemoteAddr());
        ...
        AIMRT_HL_TRACE(
            session_ptr->GetLogger(), "Http svr session custom handle request, remote addr {}, close_connect_flag: {}", session_ptr->RemoteAddr(),
            session_ptr->close_connect_flag_);

        co_await boost::beast::http::async_write(session_ptr->stream_, handle_rsp, boost::asio::use_awaitable);
  ...
  private:
    ...
    static std::string PathCat(std::string_view base, std::string_view path) {
      ...
      if (result.back() == kPathSeparator) result.resize(result.size() - 1);
      result.append(path.data(), path.size());
#endif
      return result;
    }
    ...
    static Response<boost::beast::http::string_body> BadRequestHandle(const Request<ReqBodyType>& req, std::string_view info) {
      namespace http = boost::beast::http;
    ...
    static Response<boost::beast::http::string_body> NotFoundHandle(const Request<ReqBodyType>& req, std::string_view info) {
      namespace http = boost::beast::http;
    ...
    static Response<boost::beast::http::string_body> ServerErrorHandle(const Request<ReqBodyType>& req, std::string_view info) {
      namespace http = boost::beast::http;
