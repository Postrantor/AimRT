        ...
        [this, self]() -> Awaitable<void> {
          ...
          while (state_.load() == State::kStart) {
            try {
              ...
              auto session_ptr = std::make_shared<Session>(io_ptr_, logger_ptr_, msg_handle_ptr_);
              ...
              AIMRT_TRACE("WebSocket svr accept connection get exception and exit, exception info: {}", e.what());
            }
          }
        ...
        [this, self]() -> Awaitable<void> {
          while (state_.load() == State::kStart) {
            try {
              ...
              AIMRT_TRACE("WebSocket svr timer get exception and exit, exception info: {}", e.what());
            }
          }
    ...
    boost::asio::dispatch(mgr_strand_, [this, self]() {
      ...
      while (stop_step) {
        try {
          ...
          AIMRT_WARN("WebSocket svr stop get exception at step {}, exception info: {}", stop_step, e.what());
          ++stop_step;
        }
      }
  ...
  void SendMsg(const Tcp::endpoint& ep, const std::shared_ptr<Streambuf>& msg_buf_ptr) {
    ...
    boost::asio::dispatch(mgr_strand_, [this, self, ep, msg_buf_ptr]() {
      ...
      if (finditr == session_ptr_map_.end()) {
        AIMRT_WARN("WebSocket svr can not find endpoint {} in session map", aimrt::common::util::SSToString(ep));
        return;
      }
...
private:
  ...
  struct SessionOptions {
    explicit SessionOptions(const Options& options) : max_no_data_duration(options.max_no_data_duration), max_recv_size(options.max_recv_size), binary_mode(options.binary_mode) {}
      ...
      boost::asio::co_spawn(
          ...
          [this, self]() -> Awaitable<void> {
            try {
              ...
              stream_.set_option(websocket::stream_base::timeout::suggested(boost::beast::role_type::server));

              stream_.set_option(websocket::stream_base::decorator(
                  [](websocket::response_type& res) { res.set(http::field::server, std::string(BOOST_BEAST_VERSION_STRING) + " websocket-server-coro"); }));
                    ...
                    try {
                      while (state_.load() == SessionState::kStart) {
                        while (!data_list_.empty()) {
                          ...
                          tick_has_data_ = true;
                          size_t write_data_size = co_await stream_.async_write((*data_itr)->data(), boost::asio::use_awaitable);
                          AIMRT_TRACE("WebSocket svr session async write {} bytes to {}.", write_data_size, RemoteAddr());
                        ...
                        try {
                          ...
                          AIMRT_TRACE("WebSocket svr session timer canceled, remote addr {}, exception info: {}", RemoteAddr(), e.what());
                        }
                      }
                    } catch (const std::exception& e) {
                      AIMRT_WARN("WebSocket svr session send co get exception and exit, remote addr {}, exception info: {}", RemoteAddr(), e.what());
                    }
                  ...
                  [this, self]() -> Awaitable<void> {
                    try {
                      while (state_.load() == SessionState::kStart) {
                        ...
                        size_t read_data_size = co_await stream_.async_read(*msg_buf, boost::asio::use_awaitable);
                        AIMRT_TRACE("WebSocket svr session async read {} bytes from {}.", read_data_size, RemoteAddr());
                        tick_has_data_ = true;

                        boost::asio::post(*io_ptr_, [this, msg_buf]() { (*msg_handle_ptr_)(Socket().remote_endpoint(), msg_buf); });
                      }
                    } catch (const std::exception& e) {
                      AIMRT_WARN("WebSocket svr session recv co get exception and exit, remote addr {}, exception info: {}", RemoteAddr(), e.what());
                    }
              ...
              AIMRT_WARN("WebSocket svr session get exception and exit, remote addr {}, exception info: {}", RemoteAddr(), e.what());
              Shutdown();
            }
      ...
      boost::asio::co_spawn(
          ...
          [this, self]() -> Awaitable<void> {
            try {
              ...
              AIMRT_TRACE("WebSocket svr session timer get exception and exit, remote addr {}, exception info: {}", RemoteAddr(), e.what());
            }
      ...
      boost::asio::dispatch(session_socket_strand_, [this, self]() {
        ...
        while (stop_step) {
          try {
            ...
            AIMRT_TRACE("WebSocket svr session stop get exception at step {}, remote addr {}, exception info: {}", RemoteAddr(), stop_step, e.what());
            ++stop_step;
          }
        }
      });
      ...
      boost::asio::dispatch(session_mgr_strand_, [this, self]() {
        ...
        while (stop_step) {
          try {
            ...
            AIMRT_WARN("WebSocket svr session mgr stop get exception at step {}, remote addr {}, exception info: {}", stop_step, RemoteAddr(), e.what());
            ++stop_step;
          }
        }
      });
