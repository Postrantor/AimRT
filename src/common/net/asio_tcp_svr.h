...
public:
  ...
  void Initialize(const Options& options) {
    AIMRT_CHECK_ERROR_THROW(msg_handle_ptr_, "Msg handle is not set before initialize.");
        ...
        [this, self]() -> Awaitable<void> {
          ...
          while (state_.load() == State::kStart) {
            try {
              ...
              AIMRT_TRACE("Tcp svr accept a new connection, remote addr is {}", aimrt::common::util::SSToString(session_ptr->Socket().remote_endpoint()));
              session_ptr->Start();

              session_ptr_map_.emplace(session_ptr->Socket().remote_endpoint(), session_ptr);

            } catch (const std::exception& e) {
              AIMRT_TRACE("Tcp svr accept connection get exception and exit, exception info: {}", e.what());
            }
          }
        ...
        [this, self]() -> Awaitable<void> {
          while (state_.load() == State::kStart) {
            try {
              ...
              AIMRT_TRACE("Tcp svr timer get exception and exit, exception info: {}", e.what());
            }
          }
    ...
    boost::asio::dispatch(mgr_strand_, [this, self]() {
      ...
      while (stop_step) {
        try {
          ...
          AIMRT_WARN("Tcp svr stop get exception at step {}, exception info: {}", stop_step, e.what());
          ++stop_step;
        }
      }
  ...
  void SendMsg(const Tcp::endpoint& ep, const std::shared_ptr<Streambuf>& msg_buf_ptr) {
    ...
    boost::asio::dispatch(mgr_strand_, [this, self, ep, msg_buf_ptr]() {
      ...
      if (finditr == session_ptr_map_.end()) {
        AIMRT_WARN("Tcp svr can not find endpoint {} in session map", aimrt::common::util::SSToString(ep));
        return;
      }
...
private:
  ...
  struct SessionOptions {
    explicit SessionOptions(const Options& options) : max_no_data_duration(options.max_no_data_duration), max_recv_size(options.max_recv_size) {}
            ...
            try {
              while (state_.load() == SessionState::kStart) {
                while (!data_list_.empty()) {
                  ...
                  AIMRT_TRACE("Tcp svr session async write {} bytes to {}.", write_data_size, RemoteAddr());
                }
                ...
                try {
                  ...
                  AIMRT_TRACE("Tcp svr session timer canceled, remote addr {}, exception info: {}", RemoteAddr(), e.what());
                }
              }
            } catch (const std::exception& e) {
              AIMRT_WARN("Tcp svr session send co get exception and exit, remote addr {}, exception info: {}", RemoteAddr(), e.what());
            }
          ...
          [this, self]() -> Awaitable<void> {
            try {
              ...
              while (state_.load() == SessionState::kStart) {
                ...
                AIMRT_TRACE("Tcp svr session async read {} bytes from {} for head.", read_data_size, RemoteAddr());
                tick_has_data_ = true;

                AIMRT_CHECK_ERROR_THROW(
                    read_data_size == kHeadSize && head_buf[0] == kHeadByte1 && head_buf[1] == kHeadByte2,
                    "Get an invalid head, remote addr {}, read_data_size: {}, head_buf[0]: {}, head_buf[1]: {}.", RemoteAddr(), read_data_size, static_cast<uint8_t>(head_buf[0]),
                    static_cast<uint8_t>(head_buf[1]));

                uint32_t msg_len = aimrt::common::util::GetUint32FromBuf(&head_buf[2]);

                AIMRT_CHECK_ERROR_THROW(msg_len <= session_options_ptr_->max_recv_size, "Msg too large, remote addr {}, size: {}.", RemoteAddr(), msg_len);
                ...
                AIMRT_TRACE("Tcp svr session async read {} bytes from {}.", read_data_size, RemoteAddr());
                tick_has_data_ = true;
              ...
              AIMRT_WARN("Tcp svr session recv co get exception and exit, remote addr {}, exception info: {}", RemoteAddr(), e.what());
            }
      ...
      boost::asio::co_spawn(
          ...
          [this, self]() -> Awaitable<void> {
            try {
              ...
              AIMRT_TRACE("Tcp svr session timer get exception and exit, remote addr {}, exception info: {}", RemoteAddr(), e.what());
            }
      ...
      boost::asio::dispatch(session_socket_strand_, [this, self]() {
        ...
        while (stop_step) {
          try {
            ...
            AIMRT_TRACE("Tcp svr session stop get exception at step {}, remote addr {}, exception info: {}", RemoteAddr(), stop_step, e.what());
            ++stop_step;
          }
        }
      });
      ...
      boost::asio::dispatch(session_mgr_strand_, [this, self]() {
        ...
        while (stop_step) {
          try {
            ...
            AIMRT_WARN("Tcp svr session mgr stop get exception at step {}, remote addr {}, exception info: {}", stop_step, RemoteAddr(), e.what());
            ++stop_step;
          }
        }
      });
