...
void TestJoinUrl() {
  ...
  test_cases.emplace_back(TestCase{.name = "case 1", .url = Url<StringType>{}, .want_result = ""});
...
void TestParseUrl() {
  ...
  for (size_t ii = 0; ii < test_cases.size(); ++ii) {
    ...
    if (ret && cur_test_case.want_result) {
      EXPECT_EQ(ret->protocol, cur_test_case.want_result->protocol) << "Test " << cur_test_case.name << " failed, index " << ii;
      EXPECT_EQ(ret->host, cur_test_case.want_result->host) << "Test " << cur_test_case.name << " failed, index " << ii;
      EXPECT_EQ(ret->service, cur_test_case.want_result->service) << "Test " << cur_test_case.name << " failed, index " << ii;
      EXPECT_EQ(ret->path, cur_test_case.want_result->path) << "Test " << cur_test_case.name << " failed, index " << ii;
      EXPECT_EQ(ret->query, cur_test_case.want_result->query) << "Test " << cur_test_case.name << " failed, index " << ii;
      EXPECT_EQ(ret->fragment, cur_test_case.want_result->fragment) << "Test " << cur_test_case.name << " failed, index " << ii;
    }
  }
}
